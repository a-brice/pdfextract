...
def align_to_template(cform='examples/fw9.png', template='templates/fw9.png', output_file=None):
    ...
    try:
        if cv2.matchTemplate(imgRef, imgTest, method=cv2.TM_CCOEFF_NORMED)[0, 0] > 0.8:
            return imgTest
    except:
        pass
    ...
    try: 
        ...
        assert sum([x.distance for x in matches[:4]]) < 50, "Bad alignement, extraction not guarantied"
